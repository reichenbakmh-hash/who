...
function wait(ms){ return new Promise(r => setTimeout(r, ms * READ_PACE)); }
...
async function playIntro(){
  ...
  document.getElementById('introPhotoImg').src = INTRO_PHOTO_IMG;
...
function renderInstagram(){
  ...
  INSTA_POSTS.forEach(p=>{
    ...
    post.innerHTML = `
      ...
      <div class="igImg" style="background-image:url('${p.img}')" title="${p.desc}"></div>
  ...
  document.getElementById('storyArchived').onclick = () => {
    openLightbox(INSTA_STORY_DESC, INSTA_STORY_IMG);
  };
}
...
function renderFichiers(){
  ...
  PHOTOS.forEach(p=>{
    ...
    if (p.img) t.style.backgroundImage = `url('${p.img}')`;
    t.title = p.desc;
    t.addEventListener('click', ()=> openLightbox(p.desc + (p.meta ? ' — ' + p.meta : ''), p.img));
    grid.appendChild(t);
  });
...
function renderPreuves(){
  ...
  c.innerHTML = `
    <div class="proofBlock">
      ...
      <img class="proofImg" src="${PREUVES.photoImg}" alt="">
...
function openLightbox(caption, imgSrc){
  document.getElementById('lightbox').classList.remove('hidden');
  document.querySelector('.lightboxCaption').textContent = caption;
  const img = document.getElementById('lightboxImg');
  if (imgSrc){ img.src = imgSrc; img.classList.remove('hidden'); }
...
function renderPortier(){
  ...
  events.forEach(ev=>{
    ...
    const imgTag = ev.img ? `<img class="portierThumb" src="${ev.img}" alt="">` : '';
    row.innerHTML = `${imgTag}<div class="portierTime">${ev.time}</div><div class="portierDesc">${ev.desc}</div>`;
    row.addEventListener('click', ()=> openLightbox(ev.desc, ev.img));
    list.appendChild(row);
  });
}
...
async function playFinalMonologue(){
  ...
  await playFinalReveal();
}

/* ================= Révélation finale plein écran (effet de clôture) ================= */
async function playFinalReveal(){
  ensureAudio();
  const reveal = document.getElementById('finalReveal');
  const img = document.getElementById('finalRevealImg');
  const lineEl = document.getElementById('finalRevealLine');
  const titleEl = document.getElementById('finalRevealTitle');

  img.src = FINAL_REVEAL.img;
  lineEl.textContent = '';
  titleEl.textContent = '';
  titleEl.classList.remove('show');
  reveal.classList.remove('zoom','redshift','fadeImg');
  reveal.classList.remove('hidden');
  void reveal.offsetWidth;
  reveal.classList.add('show');

  await wait(1200);
  reveal.classList.add('zoom');
  tone(60, 2.2, 'sawtooth', 0.06);
  await wait(3400);

  reveal.classList.add('redshift');
  await wait(1800);

  flashWhite();
  glitchPulse(500);
  tone(1200, 0.4, 'square', 0.05);
  await wait(1200);

  await typeParagraph(lineEl, FINAL_REVEAL.line, null, 55);
  await wait(3200);

  reveal.classList.add('fadeImg');
  await wait(2600);

  titleEl.textContent = FINAL_REVEAL.title;
